...
namespace ns3
{
  ...
  {
    NS_LOG_FUNCTION(this);
    return 4 + 8 + 4 + 4 + 4 + 4 + 6; //Jerry 6 need to be deleted
  }

  //tmm
  void
  HTTPHeader::SetPacketNum(uint32_t packet_num)
  {
    NS_LOG_FUNCTION(this << packet_num);
    m_packet_num = packet_num;
  }
  uint32_t
  HTTPHeader::GetPacketNum(void) const
  {
    NS_LOG_FUNCTION(this);
    return m_packet_num;
  }
  ...
  HTTPHeader::Serialize(Buffer::Iterator start) const
  {
    ...
    i.WriteHtonU32(m_packet_num); //tmm
  }
  uint32_t
  HTTPHeader::Deserialize(Buffer::Iterator start)
  {
    ...
    m_packet_num = i.ReadNtohU32(); //tmm
